...
describe('BuildCommand', () => {
	...
	describe('script execution', () => {
		...
		it('should run build script via runScript()', async () => {
			vi.mocked(packageJson.getPackageScripts).mockResolvedValue({
				build: { command: 'tsc', source: 'package-manager' },
			})
			vi.mocked(packageManager.runScript).mockResolvedValue({})
		...
		it('should pass worktree path to runScript()', async () => {
			vi.mocked(packageJson.getPackageScripts).mockResolvedValue({
				build: { command: 'make build', source: 'iloom-config' },
			})
			vi.mocked(packageManager.runScript).mockResolvedValue({})
	...
	describe('missing script handling', () => {
		...
		it('should throw with "No build script defined in package.iloom.json" when script missing', async () => {
			vi.mocked(packageJson.getPackageScripts).mockResolvedValue({
			...
			await expect(command.execute({})).rejects.toThrow(
				'No build script defined in package.json or package.iloom.json'
			)
		})
